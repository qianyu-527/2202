<template>
	<div>
		...
		<cateUpdate ref="update" :title="title" @que="que" :flag="flag"></cateUpdate>
		<!-- 分页 -->
		 <el-pagination
...
export default {
  ...
  methods: {
	...
	que(val){
		if(this.flag==0){
			this.$http({
				...
				data:{cat_name:val.cat_name,cat_pid:0,cat_level:val.index}
			}).then(res=>{
				const {status,msg}=res.data.meta
				if(status===200){
	...
	this.$http({
				method:"put",
				url:this.$http.addorUrl(`categories/${val.flag}`),
